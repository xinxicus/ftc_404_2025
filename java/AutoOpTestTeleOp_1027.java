...
@TeleOp(name="AutoOp Feature Test 10/27")
public class AutoOpTestTeleOp_1027 extends LinearOpMode {
    // ---- Odometry constants ----
    ...
    private void turnToHeading(double targetDegrees, double speed, double timeoutSeconds) {
        ...
        while (opModeIsActive() && timer.seconds() < timeoutSeconds) {
            ...
            // PD controller with proper time-based derivative
            double currentTime = timer.seconds();
            double dt = currentTime - lastTime;
            if (dt < 0.001) dt = 0.001;  // Prevent division by zero
            
            double kP = 0.6;   // Proportional gain - responsive but stable
            double kD = 0.4;   // Derivative gain - good damping
            
            // Calculate derivative term (skip on first iteration to avoid spike)
            double errorRate = 0;
            if (!firstIteration) {
                errorRate = (headingError - lastError) / dt;
            }
            
            double turnPower = (headingError * kP) + (errorRate * kD);
            ...
            // Early exit if very close to target (prevents oscillation)
            if (Math.abs(headingErrorDeg) < 2.0) {  // Within 2 degrees, stop immediately
                break;
            }
            
            // Check if we're within threshold for multiple consecutive reads
            if (Math.abs(headingErrorDeg) < HEADING_THRESHOLD) {
                stableCount++;
                if (stableCount >= 3) {  // Must be stable for 3 consecutive reads
                    break;
                }
            } else {
                stableCount = 0;
            ...
            // Smart power management based on distance from target
            if (Math.abs(headingErrorDeg) < HEADING_THRESHOLD) {
                // Within threshold: Allow very small powers or stop completely
                if (Math.abs(turnPower) < 0.08) {
                    turnPower = 0;
                }
            } else {
                // Outside threshold: Apply minimum power to ensure robot keeps moving
                if (Math.abs(turnPower) > 0 && Math.abs(turnPower) < 0.12) {
            ...
            telemetry.addData("📍 Current", "%.1f°", Math.toDegrees(heading));
            telemetry.addData("📏 Error", "%.1f°", headingErrorDeg);
            telemetry.addData("⚡ Power", "%.3f", turnPower);
            telemetry.addData("✓ Stable", "%d/3", stableCount);
            telemetry.update();
            
            sleep(20);  // Small delay to prevent CPU overload and stabilize readings
        }
