...
@TeleOp(name="TeleOp Light Test")
public class TeleOpLightTest extends LinearOpMode {
    ...
    // Stores the current exposure index (manual control via UP/DOWN buttons)
    private int currentExposureIndex = DEFAULT_EXPOSURE_INDEX;  // Start with default (dim)
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // UP/DOWN buttons control exposure index manually
            boolean curUp = gamepad1.dpad_up || gamepad2.dpad_up;
            boolean curDown = gamepad1.dpad_down || gamepad2.dpad_down;
            
            // UP increases exposure index (towards dimmer)
            if (curUp && !prevUp) { 
                if (currentExposureIndex < EXPOSURE_LEVELS_MS.length - 1) {
                    currentExposureIndex++;
                    // Reset camera settings to force update with new index
                    currentExposureMS = -1;
                    currentGain = -1;
                    telemetry.addData("Exposure", "⬆️ Increased to [%d] %s", currentExposureIndex, LEVEL_NAMES[currentExposureIndex]);
                }
            }
            prevUp = curUp;
            
            // DOWN decreases exposure index (towards brighter)
            if (curDown && !prevDown) { 
                if (currentExposureIndex > 0) {
                    ...
                    currentExposureMS = -1;
                    currentGain = -1;
                    telemetry.addData("Exposure", "⬇️ Decreased to [%d] %s", currentExposureIndex, LEVEL_NAMES[currentExposureIndex]);
                }
            }
            prevDown = curDown;
            ...
            // Display current exposure setting (manual control)
            int exposureMS = EXPOSURE_LEVELS_MS[currentExposureIndex];
            int gain = GAIN_LEVELS[currentExposureIndex];
            String levelName = LEVEL_NAMES[currentExposureIndex];
            telemetry.addData("Exposure", "%s [%d] (exp:%dms, gain:%d)", 
                levelName, currentExposureIndex, exposureMS, gain);
            
            telemetry.addData("Hint", "A:AprilTag, B:Field/Robot, Y:Reset, UP/DOWN:Exposure, L2:Reverse");
            telemetry.addData("", "");
    ...
    */
    ...
    /**
     * Detect AprilTag with manual exposure control.
     * 
     * Uses the current exposure index set by UP/DOWN buttons.
     * - UP button increases index (towards dimmer settings)
     * - DOWN button decreases index (towards brighter settings)
     * 
    ...
    private boolean detectAprilTag() {
        ...
        // Use the current manual exposure index
        int exposureMS = EXPOSURE_LEVELS_MS[currentExposureIndex];
        int gain = GAIN_LEVELS[currentExposureIndex];
        String levelName = LEVEL_NAMES[currentExposureIndex];
        
        return tryDetectWithExposure(desiredTagId, exposureMS, gain, levelName);
    }
